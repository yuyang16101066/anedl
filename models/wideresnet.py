...
class RBF(nn.Module):
    ...
    def __init__(self, in_features, out_features, out_dim):
    ...
    def forward(self, input):
        radial_val = self.kernel_fun(input)
        out = self.linear(torch.cat([input, radial_val], dim=1))
        return out
...
class WideResNet_Open(nn.Module):
    def __init__(self, num_classes, depth=28, widen_factor=2, drop_rate=0.0):
        ...
        self.fc = nn.Linear(channels[3], num_classes)
        self.fc_cp = nn.Linear(channels[3], num_classes)
        out_open = num_classes # for dbu
        #self.fc_open = nn.Linear(channels[3], out_open, bias=False)
        self.fc_open = nn.Sequential(
                nn.Linear(channels[3], channels[3]),
                nn.BatchNorm1d(channels[3], momentum=0.001),
                nn.ReLU(),
                nn.Linear(channels[3], channels[3]),
                nn.BatchNorm1d(channels[3], momentum=0.001),
                nn.ReLU(),
                ...
                nn.BatchNorm1d(channels[3], momentum=0.001),
                nn.ReLU(),
                nn.Linear(channels[3], out_open, bias=False))
        #self.fc_open_rbf = RBF(channels[3], channels[3], out_open)
    ...
    def forward(self, x, feature=False, feat_only=False, fc_cp=False):
        #self.weight_norm()
        out = self.conv1(x)
        out = self.block1(out)
        out = self.block2(out)
        ...
        #out_open = self.fc_open_rbf(out_open)
        out_open = self.softplus(out_open) + 1.0
        if not fc_cp:
            if feature:
            ...
            self._sync_fc()
            if feature:
                return self.fc_cp(out), out_open, out
            else:
                return self.fc_cp(out), out_open

    def fc_open_forward(self, feat):
        out = self.fc_open(feat)
        out = self.softplus(out) + 1.0
        return out

    def _sync_fc(self):
        for param_q, param_k in zip(self.fc.parameters(), self.fc_cp.parameters()):
